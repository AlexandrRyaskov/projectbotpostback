...
async def send_msg(message: Message):
    ...
    async with create_connection() as conn:
        user = await get_user(conn, tg_user_id=message.from_user.id)
        if user is None:
            await create_user(
                conn,
                User(tg_id=message.from_user.id, status=UserStatus.INIT),
            )
...
async def registration_successful(call: CallbackQuery):
    ...
    async with create_connection() as conn:
        user = await get_user(conn, tg_user_id=call.from_user.id)
